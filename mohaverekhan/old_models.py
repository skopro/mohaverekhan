...
class ManualTagger(models.Model):
    ...
    sentences = models.ArrayReferenceField(to=Sentence, on_delete=models.CASCADE null=True, blank=True, default=[])

    class Meta:
        abstract = True

    def __str__(self):
        return ' '.join([sentence.__str__() for sentence in self.sentences])

class AutomaticTagger(models.Model):
    sentences = models.ArrayModelField(model_container=Sentence, null=True, blank=True, default=[])
...
class ManualFormalizer(models.Model):
    content = NonStrippingTextField()
    normalized = NonStrippingTextField(blank=True)
    manual_tagger = models.EmbeddedModelField(model_container=ManualTagger, null=True, blank=True)
    automatic_tagger = models.EmbeddedModelField(model_container=AutomaticTagger, null=True, blank=True)
...
class AutomaticFormalizer(models.Model):
    content = NonStrippingTextField()
    normalized = NonStrippingTextField(blank=True)
    manual_tagger = models.EmbeddedModelField(model_container=ManualTagger, null=True, blank=True)
    automatic_tagger = models.EmbeddedModelField(model_container=AutomaticTagger, null=True, blank=True)
...
class Text(models.Model):
    ...
    manual_tagger = models.EmbeddedModelField(model_container=ManualTagger, null=True, blank=True)
    automatic_tagger = models.EmbeddedModelField(model_container=AutomaticTagger, null=True, blank=True)
    manual_formalizer = models.EmbeddedModelField(model_container=ManualFormalizer, null=True, blank=True)
    automatic_formalizer = models.EmbeddedModelField(model_container=AutomaticFormalizer, null=True, blank=True)
    has_manual_tagger = models.BooleanField(blank=True, default=False)
    has_automatic_tagger = models.BooleanField(blank=True, default=False)
    has_manual_formalizer = models.BooleanField(blank=True, default=False)
    has_automatic_formalizer = models.BooleanField(blank=True, default=False)
    has_manual_formalizer_manual_tagger = models.BooleanField(blank=True, default=False)
    has_manual_formalizer_automatic_tagger = models.BooleanField(blank=True, default=False)
    has_automatic_formalizer_manual_tagger = models.BooleanField(blank=True, default=False)
    has_automatic_formalizer_automatic_tagger = models.BooleanField(blank=True, default=False)
    ...
    def set_booleans(self):
        if self.manual_tagger is not None and self.manual_tagger.sentences is not None and self.manual_tagger.sentences:
            self.has_manual_tagger = True

        if self.automatic_tagger is not None and self.automatic_tagger.sentences is not None and self.automatic_tagger.sentences:
            self.has_automatic_tagger = True

        if self.manual_formalizer is not None and not self.manual_formalizer.content.isspace():
            self.has_manual_formalizer = True

        if self.automatic_formalizer is not None and not self.automatic_formalizer.content.isspace():
            self.has_automatic_formalizer = True

        if self.has_manual_formalizer:
            if (self.manual_formalizer.manual_tagger.sentences is not None
                                            and self.manual_formalizer.manual_tagger.sentences):
                self.has_manual_formalizer_manual_tagger = True

            if (self.manual_formalizer.automatic_tagger.sentences is not None
                                            and self.manual_formalizer.automatic_tagger.sentences):
                self.has_manual_formalizer_automatic_tagger = True
        
        if self.has_automatic_formalizer:
            if (self.automatic_formalizer.manual_tagger.sentences is not None
                                            and self.automatic_formalizer.manual_tagger.sentences):
                self.has_automatic_formalizer_manual_tagger = True
                
            if (self.automatic_formalizer.automatic_tagger.sentences is not None
                                            and self.automatic_formalizer.automatic_tagger.sentences):
                self.has_automatic_formalizer_automatic_tagger = True
